...
if [ ! -f "/etc/varnish/default.vcl" ]; then
    ...
    /bin/cp $CODEDEPLOY/deployment/configs/varnishcache_varnish5.repo /etc/yum.repos.d/varnishcache_varnish5.repo

    yum -q makecache -y --disablerepo='*' --enablerepo='varnishcache_varnish5'
    yum -y install varnish

    /bin/cp $CODEDEPLOY/deployment/configs/varnish-default.vcl /etc/varnish/default.vcl
